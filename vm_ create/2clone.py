...
def main():
    ...
    vc_host = "192.168.29.23"
    vc_user = "administrator@vsphere.local"
    vc_password = "Mandao#h3c64"
    ...
    des_file = "/tmp/reconfig.sh"

    destination_vm_begin = "web"
    vlan_begin = "VLAN"
    ...
    help = """
        执行方法： python %s template_name ip_range
            template_name: 模版主机名称
            ip_range: 可以为单独的 IP 地址,也可以为 IP 范围: 192.168.1.1-10
    """ % sys.argv[0]

    vm_list = create_server_list(destination_vm_begin, ip_range)

    if not vm_list:
        print(help)
        return

    vc = VmwareApi(host=vc_host, user=vc_user, password=vc_password)
    ...
    for vm_info in vm_list:    
        ...
        if 1 == 1:
            ...
            vlan_name = vlan_begin + vm_info["vlan"]
            ...
            upload_file_code = vc.upload_file(vm_name, vm_user, vm_pwd, source_file, des_file)
            if upload_file_code == 200:
                arguments = "/tmp/reconfig.sh %s %s" % (vm_info["hostname"], vm_info["ip"])
                process_status = vc.process(vm_info["hostname"], vm_user, vm_pwd, executable_program, arguments)
                if process_status:
